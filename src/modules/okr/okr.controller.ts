...
export class OkrController {
  ...
  @ApiOperation({
    ...
    description:
      '`Objective` type `DEPARTMENT`, `status`/`progress` mặc định trong service. Lỗi DB → 500.',
  })
  ...
  @ApiInternalServerErrorResponse({
    description: 'Lỗi khi lưu (package trong `createDepartmentOkr`).',
  })
  ...
  @ApiOperation({
    ...
    description: '`type: DEPARTMENT`, kèm `keyResults`, sort `createdAt DESC`.',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description: '`userId` từ JWT (`id` hoặc `sub`).',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description: 'Dành cho quy trình duyệt; relations `user`, department, managementPosition.',
  })
  @ApiOkResponse({
    type: UserOkrSwaggerDto,
    isArray: true,
  ...
  @ApiOperation({
    ...
    description: 'Chỉ bản ghi khớp `id` + `userId` JWT; `status` → `ACCEPTED`.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Append vào `proposedChanges[itemId]`. Sender `USER` → status `NEGOTIATING`; `MANAGER` → `PENDING`.',
  })
  @ApiParam({ name: 'id', format: 'uuid', description: '`user_okrs.id`.' })
  ...
  @ApiOperation({
    ...
    description: '`status` → `ACCEPTED`, gửi notification cho user.',
  })
  @ApiParam({ name: 'id', format: 'uuid' })
  @ApiOkResponse({ type: UserOkrSwaggerDto })
  @ApiNotFoundResponse({ description: '`OKR not found`.' })
  ...
  @ApiOperation({
    ...
    description: '`status` → `PENDING`, xóa `proposedChanges`, notification kèm `reason`.',
  })
  @ApiParam({ name: 'id', format: 'uuid' })
  ...
  @ApiOperation({
    ...
    description:
      'Yêu cầu `status === ACCEPTED`. Cập nhật `selfReportData`, `totalScore`, `status` → `SUBMITTED`.',
  })
  @ApiParam({ name: 'id', format: 'uuid' })
  @ApiBody({ type: SelfReportBodyDto })
  @ApiOkResponse({ type: UserOkrSwaggerDto })
  @ApiBadRequestResponse({
      description: '`OKR chưa được chấp nhận, không thể tự khai.`',
  })
  @ApiNotFoundResponse({ description: '`OKR not found`.' })
  @ApiInternalServerErrorResponse()
  ...
  @ApiOperation({
    ...
    description: 'Cùng query `getSubmittedOkrs` với status `COMPLETED`.',
  })
  @ApiOkResponse({ type: UserOkrSwaggerDto, isArray: true })
  @ApiInternalServerErrorResponse()
  ...
  @ApiOperation({
    ...
    description:
      'Lưu `managerReportData`, `managerScore`, `status` → `COMPLETED`, sync phiếu đánh giá + notification.',
  })
  @ApiParam({ name: 'id', format: 'uuid' })
  ...
  @ApiOperation({
    ...
    description: '`status` → `SUBMITTED` trên `user_evaluations`.',
  })
  ...
  @ApiOperation({
    ...
    description:
      '**Lưu ý:** không filter `SUBMITTED` — service trả toàn bộ bản ghi, sort `updatedAt DESC`.',
  })
  ...
  @ApiOperation({
    ...
    description: '`status` → `EVALUATED`, cập nhật `managerComment` / `managerRating`.',
  })
  @ApiParam({ name: 'id', format: 'uuid', description: '`user_evaluations.id`.' })
